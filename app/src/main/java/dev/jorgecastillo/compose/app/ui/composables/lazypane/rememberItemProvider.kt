...
internal fun rememberItemProvider(
    state: LazyListState, content: LazyPaneScope.() -> Unit
    ...
    val nearestItemsRangeState = remember(state) {
        mutableStateOf(Snapshot.withoutReadObservation {
    ...
    return remember(nearestItemsRangeState) {
        LazyListItemProviderImpl(derivedStateOf {
            val listScope = LazyPaneScopeImpl().apply(latestContent.value)
            LazyListItemsSnapshot(
                listScope.intervals, nearestItemsRangeState.value
            )
        })
    }
}
...
internal class LazyListItemsSnapshot(
    private val intervals: IntervalList<LazyListIntervalContent>, nearestItemsRange: IntRange
) {
    val itemsCount get() = intervals.size

    fun getKey(indexRow: Int, indexColumn: Int): Any {
        val interval = intervals[index]
        val localIntervalIndex = index - interval.startIndex
        val key = interval.value.key?.invoke(localIntervalIndex)
        return key ?: getDefaultLazyLayoutKey(indexRow, indexColumn)
    }
...
internal fun generateKeyToIndexMap(
    range: IntRange, list: IntervalList<LazyListIntervalContent>
): Map<Any, Int> {
...
private fun calculateNearestItemsRange(firstVisibleItem: Int): IntRange {
    val slidingWindowStart =
        VisibleItemsSlidingWindowSize * (firstVisibleItem / VisibleItemsSlidingWindowSize)
